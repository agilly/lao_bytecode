...
with open("bitmap_data.h", "w", encoding="utf-8") as header_file:
    ...
    header_file.write(f'static const glyph_bitmaps[{len(bitmap_dict) * len(hex_repr)}] PROGMEM = {{ \n')
    for char, hex_data in list(bitmap_dict.items())[:-1]:
        header_file.write(f'{hex_data},\n')
    header_file.write(f'{list(bitmap_dict.items())[-1][1]}}};\n')
