...
def render_text_with_harfbuzz_freetype(text: str, font_path: str, output_path: str = "harfbuzz_freetype_rendered.png"):
    ...
    try:
        ...
        total_x_advance = sum(pos.x_advance for pos in positions) // 64
        ascent = face.size.ascender >> 6
        descent = - (face.size.descender >> 6)
        image_width = total_x_advance + 100  # extra padding
        image_height = ascent + descent + 50
...
if __name__ == "__main__":
    ...
    render_text_with_harfbuzz_freetype(text, font_path)
