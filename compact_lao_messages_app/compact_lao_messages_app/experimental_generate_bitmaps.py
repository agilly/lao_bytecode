...
def generate_bitmaps_for_chars(char_list, GLYPH_WIDTH = 30, GLYPH_HEIGHT = 30, font_path="./font_files/NotoSansLao-Regular.ttf", output_header="./arduino_code/glyph_bitmaps.h"):
    ...
    for char in tqdm(char_list, desc="Processing characters"):
        ...
        total_advance_x = sum(pos.x_advance for pos in positions) // 64


        image_width, image_height = (total_advance_x + 32), 100

        bitmap_width = round(GLYPH_HEIGHT*wh_ratio)
        ...
        x = (image_width - total_advance_x) // 2
        y = 80 
        ...
        img_resized = image.resize((round(GLYPH_HEIGHT*wh_ratio), GLYPH_HEIGHT), Image.Resampling.NEAREST)
        ...
        for y_row in range(GLYPH_HEIGHT):
            byte = 0
            bits_filled = 0
            for x_col in range(GLYPH_HEIGHT*wh_ratio):
